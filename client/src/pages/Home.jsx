...
const countryFlags = {
  ...
  Taiwan: "🇹🇼", "South Korea": "🇰🇷", "Saudi Arabia": "🇸🇦",
};
...
const Home = () => {
  ...
  const avgWeeklyReturn = companies.length
    ? companies.reduce((sum, c) => sum + (c.weeklyReturn || 0), 0) / companies.length
    : 0;
  const topSector = sectors.reduce((max, s) => (s.marketWeight > (max?.marketWeight || 0) ? s : max), null);
  const topCountry = countries.reduce((max, c) => (c.totalCap > (max?.totalCap || 0) ? c : max), null);

  return (
    <div className="px-6 py-8">
      <h1 className="text-3xl font-bold mb-6 text-center">Asia Market Dashboard</h1>

      {/* Summary Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        <div className="bg-white rounded-xl shadow p-5 text-center">
          <div className="text-gray-500 text-sm mb-1">Total Market Cap</div>
        ...
        <div className="bg-white rounded-xl shadow p-5 text-center">
          ...
          <div className={`text-2xl font-bold ${avgWeeklyReturn >= 0 ? "text-green-600" : "text-red-600"}`}>
        ...
        <div className="bg-white rounded-xl shadow p-5 text-center">
          ...
          <div className="text-gray-400 text-sm">{topSector ? topSector.marketWeight.toFixed(1) + "%" : ""}</div>
        </div>
        <div className="bg-white rounded-xl shadow p-5 text-center">
          <div className="text-gray-500 text-sm mb-1">Top Country</div>
          <div className="text-xl font-bold">
            {topCountry ? (countryFlags[topCountry.country] || "") + " " + topCountry.country : "—"}
          </div>
          <div className="text-gray-400 text-sm">
            {topCountry ? (topCountry.totalCap / 1e9).toFixed(0) + "B$" : ""}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mb-10">
        {countries.map((c) => (
          ...
          >
            <span className="text-3xl mb-2">{countryFlags[c.country] || "🌏"}</span>
            <span className="font-semibold text-sm group-hover:text-blue-600 transition">{c.country}</span>
            <span className="text-xs text-gray-500 mt-1">{(c.totalCap / 1e9).toFixed(0)}B$</span>
            <span className={`text-xs font-medium mt-1 ${c.averageWeeklyReturn >= 0 ? "text-green-600" : "text-red-500"}`}>
              {c.averageWeeklyReturn >= 0 ? "▲" : "▼"} {Math.abs(c.averageWeeklyReturn).toFixed(2)}%
            </span>
          </Link>
        ))}
      </div>
            ...
            <tbody>
              {topMovers.slice(0, 5).map((c, idx) => (
                <tr key={c.ticker} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  ...
                  <td className="px-3 py-2 text-gray-500">{countryFlags[c.country] || ""} {c.country}</td>
                  <td className={`px-3 py-2 text-right font-medium ${c.weeklyReturn >= 0 ? "text-green-600" : "text-red-500"}`}>
            ...
            <tbody>
              {sectorLeaders.map((c, idx) => (
                <tr key={c.ticker} className={idx % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                  ...
                  <td className="px-3 py-2">{c.name}</td>
                  <td className="px-3 py-2 text-right">{(c.marketCap / 1e9).toFixed(1)}B$</td>
                </tr>
              ))}
            </tbody>
          </table>
